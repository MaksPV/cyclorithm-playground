...
таймлайн-точки + таблица событий справа, ошибки — панелью с выделением строки.
...
- Таймлайн — только точки событий. Прямоугольники циклов требуют знания,
  какой экземпляр какого цикла породил событие, — `Event` ядра этого не несёт.
  Нужна доработка движка (поле источника в событии), после — v2.
